...
internal static class FrontlineSpecialCombatTargetDetector
{
    ...
    [
        (FrontlineConstants.IcedotomeIrisModelCharaId, "Icedotome Iris", FrontlineFields.ShatterTerritoryId),
        (FrontlineConstants.AssaultDroneModelCharaId, "Assault Drone", FrontlineFields.ConquestTerritoryId),
        (FrontlineConstants.AssaultSystemModelCharaId, "Assault System", null),
    ];
    ...
    private static bool TryMatch(IGameObject obj, out string debugName)
    {
        ...
        foreach (var (modelCharaId, name, territoryOnly) in Targets)
        {
            if (territoryOnly is uint only && territoryId != only)
                continue;
